...
if ($method === "POST") {
    ...
    $can_edit_comment = empty($_POST["can_add_comment"]) ? "no" : $_POST["can_add_comment"];;
    $can_delete_comment = empty($_POST["can_add_comment"]) ? "no" : $_POST["can_add_comment"];;
    $can_set_status = empty($_POST["can_add_comment"]) ? "no" : $_POST["can_add_comment"];;
   
    $sql = "INSERT INTO Lauviah_board_status(name, can_add_comment, can_edit_comment, can_delete_comment, can_set_status)  VALUES ()";
    $result = new SQLquery($sql, "sisss", array($name, $can_add_comment, $can_edit_comment, $can_edit_comment, $set_status));
    print_r($result);
}
