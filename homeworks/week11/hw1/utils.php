...
class User {
    ...
    function __construct ($id) {
        if (empty($id)) {
            $this->auth = new Auth(null, null, null, null);
            return;
        } else {
        }
        $sql = "SELECT *, Lauviah_board_status.name as status_name FROM Lauviah_board_users NATURAL LEFT JOIN Lauviah_board_status WHERE id = ?";
